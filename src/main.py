...
class SpriteStacker(tk.Tk):
    ...
    def create_context_menu(self):
        ...
        self.tree_view_menu.add_command(
            label="Remove", command=self.remove_selected_items
        )
    ...
    def save_image(self):
        ...
        file_path = filedialog.asksaveasfilename(
            title="Save image as", filetypes=file_types, defaultextension=".png"
        )
    ...
    def load_and_display_images(self):
        ...
        if not self.image_paths:
            placeholder_image = Image.new(
                "RGBA", (preview_max_width, preview_max_height), (255, 255, 255, 0)
            )
            ...
            for img in images:
                ...
                temp_image.paste(
                    img, (x_offset, y_offset), img if img.mode == "RGBA" else None
                )
                combined_image.alpha_composite(temp_image)

            if max_width > preview_max_width or max_height > preview_max_height:
                combined_image.thumbnail(
                    (preview_max_width, preview_max_height), Image.LANCZOS
                )
    ...
    def on_drag_motion(self, event):
        ...
        if item and item != self.dragged_item:
            self.tree_view.move(
                self.dragged_item,
                self.tree_view.parent(item),
                self.tree_view.index(item),
            )
